...
fn main() {
    for line in stdin().lock().lines() {
        ...
        let command = match read_state_from_json_file(&format!("./rounds/{}/state.json", round_number)) {
    ...
    match state.active_worm() {
        Some(worm) => {
            if let Some(direction) = find_worm_in_firing_distance(&state, worm) {
                ...
                let choice = choices.choose(&mut rand::thread_rng()).expect("No valid directions to move in");
